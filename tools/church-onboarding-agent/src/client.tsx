...
function App() {
  ...
  return <main className="shell">
    ...
    <section className="card">
      ...
      {step === 2 && <><p className="eyebrow">Visual system</p><h2>Confirm the church style sheet</h2><p className="lede">These values create a small override file while preserving the shared resource structure and print rules.</p><div className="colors">{(["primaryColor", "secondaryColor", "accentColor", "backgroundColor", "textColor"] as const).map((key) => <Field key={key} label={key.replace("Color", " color")} type="color" value={brand[key]} onChange={(value) => setBrand({ [key]: value })} />)}</div><div className="grid"><Field label="Heading font stack" value={brand.headingFont} onChange={(headingFont) => setBrand({ headingFont })} /><Field label="Body font stack" value={brand.bodyFont} onChange={(bodyFont) => setBrand({ bodyFont })} /></div><label><span>Visual notes and logo guidance</span><textarea value={brand.visualNotes} onChange={(event) => setBrand({ visualNotes: event.target.value })} /></label><LogoUploads state={state} agentName={agentName} setError={setError} /><footer><button className="primary" disabled={busy} onClick={() => run(() => agent.stub.saveBrand(brand), 3)}>Generate style sheet</button></footer></>}
