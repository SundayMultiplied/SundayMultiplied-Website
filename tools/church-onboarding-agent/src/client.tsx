...
function OnboardingApp() {
  ...
  return <main className={step === 2 ? "shell shell--theme" : "shell"}>
    ...
    <section className={step === 2 ? "card card--theme" : "card"}>
      ...
      {step === 3 && <Approval state={state} setState={setState} busy={busy} save={() => run(() => agent.stub.saveApproval(state.reviewers, state.resources, state.deliveryDay, state.familyWorship || { style: "blended", platform: "youtube" }, state.scripture || { translation: "BSB", displayMode: "automatic", provider: "bible_gateway" }), 4)} />}
...
function Approval({ state, setState, busy, save }: { state: OnboardingState; setState: (state: OnboardingState) => void; busy: boolean; save: () => void }) {
  ...
  const familyWorship = state.familyWorship || { style: "blended", platform: "youtube" };
  const scripture = state.scripture || { translation: "BSB" as const, displayMode: "automatic" as const, provider: "bible_gateway" as const };
  return <><p className="eyebrow">Approval</p><h2>Choose who signs off</h2><p className="lede">This reviewer receives one secure link with Preview buttons, Approve all, and separate feedback for each selected resource.</p><div className="grid"><Field label="Reviewer name" value={reviewer.name} onChange={(name) => setReviewer({ name })} /><Field label="Reviewer email" type="email" value={reviewer.email} onChange={(email) => setReviewer({ email })} /><Field label="Role" value={reviewer.role} onChange={(role) => setReviewer({ role })} /><Field label="Target delivery day" value={state.deliveryDay} onChange={(deliveryDay) => setState({ ...state, deliveryDay })} /></div><fieldset><legend>Weekly resources</legend>{(["monday", "group", "family"] as ResourceType[]).map((resource) => <label className="check" key={resource}><input type="checkbox" checked={state.resources.includes(resource)} onChange={() => toggle(resource)} />{resource[0].toUpperCase() + resource.slice(1)} Multiplied</label>)}</fieldset><fieldset><legend>Scripture defaults</legend><p className="lede">Choose the church&apos;s preferred translation and normal passage-display behavior. Automatic mode shows the complete BSB passage through 12 verses and 3,000 characters; longer passages link to Bible Gateway instead.</p><div className="grid"><SelectField label="Preferred translation" value={scripture.translation} onChange={(translation) => setState({ ...state, scripture: { ...scripture, translation } })} options={[["Berean Standard Bible (BSB)", "BSB"], ["English Standard Version (ESV)", "ESV"], ["New International Version (NIV)", "NIV"], ["New Living Translation (NLT)", "NLT"], ["Christian Standard Bible (CSB)", "CSB"], ["New American Standard Bible (NASB)", "NASB"], ["New King James Version (NKJV)", "NKJV"], ["King James Version (KJV)", "KJV"]]} /><SelectField label="Passage display" value={scripture.displayMode} onChange={(displayMode) => setState({ ...state, scripture: { ...scripture, displayMode } })} options={[["Automatic by passage length", "automatic"], ["Always show full text", "full_text"], ["Reference and link only", "reference_link"]]} /></div></fieldset>{state.resources.includes("family") && <fieldset><legend>Family worship defaults</legend><p className="lede">Choose the normal song style and listening platform. Production can override these for an individual sermon.</p><div className="grid"><SelectField label="Song style" value={familyWorship.style} onChange={(style) => setState({ ...state, familyWorship: { ...familyWorship, style } })} options={[["Contemporary worship", "contemporary"], ["Hymn", "hymn"], ["Contemporary or hymn", "blended"], ["No song recommendation", "none"]]} /><SelectField label="Listening platform" value={familyWorship.platform} onChange={(platform) => setState({ ...state, familyWorship: { ...familyWorship, platform } })} options={[["YouTube", "youtube"], ["Spotify", "spotify"], ["Apple Music", "apple_music"]]} /></div></fieldset>}<footer><button className="primary" disabled={busy} onClick={save}>Confirm approval setup</button></footer></>;
}
