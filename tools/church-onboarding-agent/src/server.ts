...
export class ChurchOnboardingAgent extends Agent<AppEnv, OnboardingState> {
  ...
  async removeLogo(kind: "primary" | "reverse" | "mark" | "favicon") {
    ...
    this.save({ assets, checklist: { ...this.state.checklist, logos: assets.some((item) => item.kind === "primary") } });
    return this.state;
  }
  ...
  async researchWebsite() {
    ...
    this.save({
      ...
      brand: normalizeBrandProfile(result.brandAnalysis.suggestedProfile),
      links,
  ...
  @callable()
  async loadExistingTheme(slug: string) {
    return loadChurchTheme(this.githubConfig(), slug);
  }

  @callable()
  async createThemePullRequest(slug: string, brand: BrandProfile) {
    if (!slugPattern.test(slug)) throw new Error("Enter a valid church slug.");
    await this.saveBrandForThemeOnly(brand);
    return createThemeUpdatePullRequest(this.githubConfig(), slug, normalizeBrandProfile(brand));
  }

  private async saveBrandForThemeOnly(brand: BrandProfile) {
    const resolved = normalizeBrandProfile(brand);
    const color = /^#[0-9a-f]{6}$/i;
    const colors = [resolved.primaryColor, resolved.secondaryColor, resolved.accentColor, resolved.backgroundColor, resolved.textColor, resolved.mutedColor, resolved.borderColor, resolved.sectionBackgroundColor, resolved.calloutBackgroundColor, resolved.headerBackgroundColor, resolved.headerTextColor, resolved.scriptureBackgroundColor, resolved.questionBackgroundColor, resolved.prayerBackgroundColor, resolved.prayerTextColor];
    if (colors.some((value) => !color.test(value))) throw new Error("Theme colors must use six-digit hex values.");
  ...
  @callable()
  repositoryPreview() { return buildRepositoryFiles(this.state); }
  ...
  async createGitHubPullRequest() {
    ...
    this.ctx.waitUntil(this.syncCrm({ stage: "Repository PR", brandProfile: "Approved", repositoryWorkspace: "PR Open", onboardingDraftUrl: result.pullRequestUrl }));
    return result;
  }
}
