...
.sm-section--big-idea, .sm-section--reflection { border-left: 0; border-top: 1px solid ${b.primaryColor}; border-bottom: 1px solid ${b.primaryColor}; background: transparent; padding: 18px 0; }
.sm-question-group, .sm-resource__card, .sm-resource__scripture, .sm-resource__practice { background: transparent; }
.sm-section--prayer { border: 1px solid ${b.primaryColor}; }
`;
  if (theme === "modern") return `
/* Modern composition */
.sm-header, .sm-header.sm-header--with-logo { border-bottom: 0; border-left: 10px solid ${b.accentColor}; padding: 28px 30px; }
...
.sm-section--big-idea, .sm-section--reflection { border-left: 0; border-top: 6px solid ${b.primaryColor}; border-bottom: 1px solid ${b.borderColor}; background: transparent; padding: 24px 0; }
.sm-question-group { border: 0; border-left: 5px solid ${b.accentColor}; padding-left: 26px; background: transparent; }
...
.sm-section--big-idea, .sm-section--reflection { border: 2px solid ${b.primaryColor}; border-left-width: 2px; border-radius: 0; box-shadow: 8px 8px 0 ${b.secondaryColor}; background: ${b.backgroundColor}; }
.sm-question-group, .sm-resource__card, .sm-resource__scripture, .sm-resource__practice { border-width: 2px; background: ${b.backgroundColor}; }
...
export function buildThemeCss(churchName: string, profile: BrandProfile): string {
  ...
  const headerBackground = b.headerStyle === "plain" ? b.backgroundColor : b.headerBackgroundColor;
  const headerText = b.headerStyle === "plain" ? b.textColor : b.headerTextColor;
...
.sm-header, .sm-header.sm-header--with-logo {
  margin-bottom: 38px;
  padding: ${b.headerStyle === "filled" ? "24px" : b.headerStyle === "split" ? "24px 0" : "0 0 24px"};
  background: ${headerBackground};
  color: ${headerText};
  border-bottom: 4px solid ${b.primaryColor};
  border-radius: ${b.headerStyle === "filled" ? b.cornerRadius : "0"};
}
...
.sm-section h2, .sm-section h3, .sm-resource h1, .sm-resource h2, .sm-resource h3 {
  color: ${b.primaryColor};
  font-family: ${b.headingFont};
  font-weight: ${b.headingWeight};
  text-transform: ${b.headingTransform};
}
.sm-section h3 { color: ${b.accentColor}; }
.sm-section li::marker { color: ${b.accentColor}; font-weight: 800; }

.sm-section--big-idea, .sm-section--reflection {
  padding: 22px 24px;
  border-left: 6px solid ${b.accentColor};
  border-radius: ${b.cornerRadius};
  background: ${b.calloutBackgroundColor};
}
.sm-section--scripture {
  padding: 24px;
...
.sm-scripture-text { padding: 18px 20px; border-left: 4px solid ${b.accentColor}; background: ${b.scriptureBackgroundColor}; }
.sm-scripture-attribution { color: ${b.mutedColor}; }

.sm-question-group, .sm-resource__card, .sm-resource__scripture, .sm-resource__practice {
  padding: 20px 22px;
  border: ${cardBorder};
  border-radius: ${b.cornerRadius};
  background: ${b.questionBackgroundColor || cardBackground};
}
.sm-document--monday .sm-section--takeaways,
.sm-document--group .sm-section--key-moments {
  padding: 22px 24px;
  border: ${cardBorder};
  border-radius: ${b.cornerRadius};
  background: ${cardBackground};
}
.sm-document--group .sm-section--midweek, .sm-document--group .sm-section--leader-tip {
  padding: 20px 22px;
  border-radius: ${b.cornerRadius};
  background: ${b.calloutBackgroundColor};
}
.sm-section--prayer {
  padding: 22px 24px;
  border-radius: ${b.cornerRadius};
  background: ${b.prayerBackgroundColor};
  color: ${b.prayerTextColor};
}
...
@media (max-width: 680px) {
  ...
  .sm-header-logo-wrap { order: -1; }
  .sm-church-logo { max-width: 165px; max-height: 62px; }
}
