...
export function ProductionDashboard() {
  ...
  const scopedJobCount = useMemo(
    () => churchFilter ? jobs.filter((job) => job.churchSlug === churchFilter).length : jobs.length,
    [jobs, churchFilter],
  );
  ...
  return <main className="approval-dashboard">
    ...
    <section className="approval-create production-queue">
      ...
      {jobs.length > 0 && <div className="production-queue-controls" aria-label="Production queue controls">
        ...
        <div className="production-filter-summary"><span>Showing <strong>{visibleJobs.length}</strong> of <strong>{scopedJobCount}</strong></span>{(churchFilter || seriesFilter || queueSort !== "newest") && <button type="button" onClick={() => { setChurchFilter(""); setSeriesFilter(""); setQueueSort("newest"); }}>Clear</button>}</div>
      </div>}
      {jobs.length === 0 ? <p>No sermon production jobs yet.</p> : visibleJobs.length === 0 ? <div className="production-filter-empty"><strong>No matching production jobs</strong><p>Clear or change the church and sermon-series filters.</p></div> : <div className="approval-table production-job-table">
        <div className="approval-table-row approval-table-labels"><span>Select</span><span>Church</span><span>Sermon</span><span>Series</span><span>Date</span><span>Metadata</span><span>Resources</span><span>Action</span></div>
        {visibleJobs.map((job) => { const revisionRequested = hasPendingRevision(job.id); return <div className="approval-table-row" key={job.id}>
          ...
          <span className="production-church-cell"><strong>{job.churchName}</strong></span>
          <span className="production-sermon-cell"><strong>{job.metadata.sermonTitle || "Title not detected"}</strong><small className="production-job-id" title={job.id}>Job {job.id.slice(0, 8)}</small></span>
          ...
          <span className="production-action-stack"><button type="button" className="production-edit-metadata" onClick={() => editMetadata(job)}>Edit details</button>{job.status === "awaiting_analysis_review" ? <button type="button" className="approval-approve production-primary-action" onClick={() => void openAnalysis(job)} disabled={loadingAnalysisId === job.id}>{loadingAnalysisId === job.id ? "Loading…" : "Review analysis"}</button> : <>{revisionRequested ? <><a className="approval-status status-revision_requested production-revision-link" href="/revisions">revision requested</a>{job.reviewUrl && <a href={job.reviewUrl} target="_blank" rel="noreferrer">Open review</a>}</> : job.status === "sent_for_approval" ? <><strong className="notification-sent">sent for approval</strong>{job.reviewUrl && <a href={job.reviewUrl} target="_blank" rel="noreferrer">Open review</a>}</> : <button type="button" className="approval-approve production-primary-action" onClick={() => void sendForApproval(job)} disabled={sendingId === job.id}>{sendingId === job.id ? "Sending…" : "Send for approval"}</button>}<button type="button" className="comparison-create-button" onClick={() => void createComparison(job)} disabled={comparingId === job.id}>{comparingId === job.id ? "Generating A/B/C…" : "Create A/B/C comparison"}</button></>}</span>
        </div>; })}
      </div>}
    </section>
